...
char	*ft_conver_p(void *ptr)
{
	...
	if (ptr == (void *) LONG_MIN)
		return (ft_strdup("0x8000000000000000"));
	if (ptr == (void *) ULONG_MAX)
		return (ft_strdup("0xffffffffffffffff"));
	temp_ptr = ft_conver_nbr_base((long long int) ptr, 16);
	if (temp_ptr == NULL)
		return (NULL);
	text_ptr = ft_strdup("0x");
	text_ptr = ft_recalloc((unsigned char *) text_ptr, ft_strlen(temp_ptr) + 3);
	if (text_ptr == NULL)
	{
		free(temp_ptr);
		return (NULL);
